    ...
    def __call__(self, images, boxes):
        ...
        for image, box, scale in zip(images, boxes, scale_factor):
            ...
            if new_h < 1 or new_w < 1:
                print(f"Skipping too-small crop: input size (H={h}, W={w}), scale={scale.item()}")
                continue  # or return a blank tensor
            image = F.interpolate(image.unsqueeze(0), size=(new_h, new_w))[0]
