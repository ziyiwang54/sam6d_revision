...
if __name__ == "__main__":
    ...
    # Process detections sequentially to reduce memory usage
    all_pose_scores = []
    all_pred_rot = []
    all_pred_trans = []
    
    for i in range(ninstance):
        # Create single instance data
        single_input = {}
        for key in input_data:
            if key in ['pts', 'rgb', 'rgb_choose', 'model', 'K']:
                single_input[key] = input_data[key][i:i+1]  # Keep batch dimension
            elif key == 'score':
                single_input[key] = input_data[key][i:i+1]
        
        # Add template data for single instance
        single_input['dense_po'] = all_tem_pts
        single_input['dense_fo'] = all_tem_feat
        
        with torch.no_grad():
            out_single = model(single_input)
        
        # Store results
        if 'pred_pose_score' in out_single.keys():
            pose_score = (out_single['pred_pose_score'] * out_single['score']).detach().cpu().numpy()
        else:
            pose_score = out_single['score'].detach().cpu().numpy()
        
        all_pose_scores.append(pose_score[0])
        all_pred_rot.append(out_single['pred_R'].detach().cpu().numpy()[0])
        all_pred_trans.append(out_single['pred_t'].detach().cpu().numpy()[0] * 1000)
        
        # Clear GPU cache after each instance
        torch.cuda.empty_cache()
    
    # Convert to numpy arrays
    pose_scores = np.array(all_pose_scores)
    pred_rot = np.array(all_pred_rot)
    pred_trans = np.array(all_pred_trans)
