...
def get_test_data(rgb_path, depth_path, cam_path, cad_path,
                  seg_path, det_score_thresh, cfg):
    dets = []
    with open(seg_path) as f:
        dets_ = json.load(f)               # ← ISM detections
    ...
    # ⬇️ NEW: pick the single best-scoring detection
    if len(dets) == 0:
        raise RuntimeError("No detection survives the score threshold.")
    TOP_K = 5                                # how many ISM masks to keep
    dets = sorted(dets, key=lambda d: d["score"], reverse=True)[:TOP_K]

    print(f"Number of detections after thresholding: {len(dets)}")
...
if __name__ == "__main__":
    ...
    #     tem_path, cfg.test_dataset
    # )

    # KEEP_N = 6                      # ← try 6 or even 4
    # all_tem        = all_tem[:KEEP_N]
    # all_tem_pts    = all_tem_pts[:KEEP_N]
    # all_tem_choose = all_tem_choose[:KEEP_N]
    # # -----------------------------------------------------------
    # with torch.no_grad():
